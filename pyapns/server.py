...
def encode_notifications(tokens, notifications):
  ...
  if type(notifications) is dict and type(tokens) in (str, str):
    tokens, notifications = ([tokens], [notifications])
  if type(notifications) is list and type(tokens) is list:
    return ''.join([structify(*y) for y in ((binaryify(t), json.dumps(p, separators=(',',':'), ensure_ascii=False).encode('utf-8'))
